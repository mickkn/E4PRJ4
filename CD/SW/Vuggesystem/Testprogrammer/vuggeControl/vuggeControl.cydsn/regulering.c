...
void updateRegParameter(uint8 vuggefrekvens, uint8 vuggeudsving)
{
    ...
    if(valid)
    {
        prevFrek = vuggefrekvens;
        prevUdsving = vuggeudsving;
...
fix16_t beregnRegulering(fix16_t currentAngle)
{
    ...
    fix16_t d = fix16div((e - prev_e), 328); // Derivetive (dt = 5ms)
    i_e += fix16mul(e, 328);
    
    fix16_t C = fix16mul(K_P, e) + fix16mul(K_D, d) + fix16mul(K_I, i_e);
...
/* Function Name: PWMStyring
   Summary:
     Controls the PWM module with parameter intensity
*/
void PWMStyring(fix16_t intensity)
{
    uint8 offset = 0;
    if(intensity < 0) offset = -hysterese;
    else if(intensity > 0) offset = hysterese;
    intensity /= 2;
    intensity += 6553600;
    uint8_t comp;
    if(intensity > 0) comp = (intensity >> 16) + offset;
//    if(intensity > 6553600) intensity = 6553600;
//    else if (intensity < -6553600) intensity = -6553600;
//    
