...
const uint8_t imuInitArray[IMU_INIT_LEN][2] = {
    ...
    {0x1A, 5},              //set dlpf freq to 44 and 42 for acc and gyro respectively
...
//calculated angles
fix16_t prevAng = 0, accAng = 0, estAng = 0;
...
fix16_t getVuggeUdsvingSens()
{
    ...
    // Use if forcemag is between 0.90*(2^16) and 1.4*(2^16)
    fix16_t accAng = 0;
...
//        stallCount = 0;
//    }
//    if(stallCount >= 10) setupI2CDevice(IMU_ADDRESS, imuInitArray, IMU_INIT_LEN);
//    char str[20];
//    sprintf(str, "%d, ", estang);
//    debugout_putstring(str);
    return estAng - 327680;
}
