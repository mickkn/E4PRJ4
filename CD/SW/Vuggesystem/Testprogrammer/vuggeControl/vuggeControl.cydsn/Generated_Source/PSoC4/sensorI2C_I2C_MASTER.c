/*******************************************************************************
* File Name: sensorI2C_I2C_MASTER.c
* Version 2.0
*
* Description:
...
/* Master variables */
volatile uint16 sensorI2C_mstrStatus;      /* Master Status byte  */
volatile uint8  sensorI2C_mstrControl;     /* Master Control byte */

/* Receive buffer variables */
volatile uint8 * sensorI2C_mstrRdBufPtr;   /* Pointer to Master Read buffer */
volatile uint32  sensorI2C_mstrRdBufSize;  /* Master Read buffer size       */
volatile uint32  sensorI2C_mstrRdBufIndex; /* Master Read buffer Index      */

/* Transmit buffer variables */
volatile uint8 * sensorI2C_mstrWrBufPtr;   /* Pointer to Master Write buffer */
volatile uint32  sensorI2C_mstrWrBufSize;  /* Master Write buffer size       */
volatile uint32  sensorI2C_mstrWrBufIndex; /* Master Write buffer Index      */
volatile uint32  sensorI2C_mstrWrBufIndexTmp; /* Master Write buffer Index Tmp */

#if (!sensorI2C_CY_SCBIP_V0 && \
    sensorI2C_I2C_MULTI_MASTER_SLAVE_CONST && sensorI2C_I2C_WAKE_ENABLE_CONST)
    static void sensorI2C_I2CMasterDisableEcAm(void);
#endif /* (!sensorI2C_CY_SCBIP_V0) */


/*******************************************************************************
* Function Name: sensorI2C_I2CMasterWriteBuf
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  sensorI2C_mstrStatus  - used to store current status of I2C Master.
*  sensorI2C_state       - used to store current state of software FSM.
*  sensorI2C_mstrControl - used to control master end of transaction with
*  or without the Stop generation.
*  sensorI2C_mstrWrBufPtr - used to store pointer to master write buffer.
*  sensorI2C_mstrWrBufIndex - used to current index within master write
*  buffer.
*  sensorI2C_mstrWrBufSize - used to store master write buffer size.
*
*******************************************************************************/
uint32 sensorI2C_I2CMasterWriteBuf(uint32 slaveAddress, uint8 * wrData, uint32 cnt, uint32 mode)
{
    ...
    errStatus = sensorI2C_I2C_MSTR_NOT_READY;

    if(NULL != wrData)  /* Check buffer pointer */
    {
        /* Check FSM state and bus before generating Start/ReStart condition */
        if(sensorI2C_CHECK_I2C_FSM_IDLE)
        {
            sensorI2C_DisableInt();  /* Lock from interruption */

            /* Check bus state */
            errStatus = sensorI2C_CHECK_I2C_STATUS(sensorI2C_I2C_STATUS_BUS_BUSY) ?
                            sensorI2C_I2C_MSTR_BUS_BUSY : sensorI2C_I2C_MSTR_NO_ERROR;
        }
        else if(sensorI2C_CHECK_I2C_FSM_HALT)
        {
            sensorI2C_mstrStatus &= (uint16) ~sensorI2C_I2C_MSTAT_XFER_HALT;
                              errStatus  = sensorI2C_I2C_MSTR_NO_ERROR;
        }
        else
        {
    ...
    if(sensorI2C_I2C_MSTR_NO_ERROR == errStatus) /* No error proceed */
    {
    #if (!sensorI2C_CY_SCBIP_V0 && \
        sensorI2C_I2C_MULTI_MASTER_SLAVE_CONST && sensorI2C_I2C_WAKE_ENABLE_CONST)
            sensorI2C_I2CMasterDisableEcAm();
    #endif /* (!sensorI2C_CY_SCBIP_V0) */

        /* Set up write transaction */
        sensorI2C_state = sensorI2C_I2C_FSM_MSTR_WR_ADDR;
        sensorI2C_mstrWrBufIndexTmp = 0u;
        sensorI2C_mstrWrBufIndex    = 0u;
        sensorI2C_mstrWrBufSize     = cnt;
        sensorI2C_mstrWrBufPtr      = (volatile uint8 *) wrData;
        sensorI2C_mstrControl       = (uint8) mode;

        slaveAddress = sensorI2C_GET_I2C_8BIT_ADDRESS(slaveAddress);

        sensorI2C_mstrStatus &= (uint16) ~sensorI2C_I2C_MSTAT_WR_CMPLT;
        ...
        if(sensorI2C_CHECK_I2C_MODE_RESTART(mode))
        {
            sensorI2C_I2C_MASTER_GENERATE_RESTART;
            sensorI2C_TX_FIFO_WR_REG = slaveAddress;
        }
        else
        {
            sensorI2C_TX_FIFO_WR_REG = slaveAddress;
            sensorI2C_I2C_MASTER_GENERATE_START;
        }
    ...
    sensorI2C_EnableInt();   /* Release lock */
...
/*******************************************************************************
* Function Name: sensorI2C_I2CMasterReadBuf
********************************************************************************
*
* Summary:
...
*  sensorI2C_mstrControl - used to control master end of transaction with
*  or without the Stop generation.
*  sensorI2C_mstrRdBufPtr - used to store pointer to master write buffer.
*  sensorI2C_mstrRdBufIndex - used to current index within master write
*  buffer.
*  sensorI2C_mstrRdBufSize - used to store master write buffer size.
*
*******************************************************************************/
uint32 sensorI2C_I2CMasterReadBuf(uint32 slaveAddress, uint8 * rdData, uint32 cnt, uint32 mode)
{
    ...
    if(sensorI2C_I2C_MSTR_NO_ERROR == errStatus) /* No error proceed */
    {
        ...
        sensorI2C_state = sensorI2C_I2C_FSM_MSTR_RD_ADDR;
        sensorI2C_mstrRdBufIndex = 0u;
        sensorI2C_mstrRdBufSize  = cnt;
        sensorI2C_mstrRdBufPtr   = (volatile uint8 *) rdData;
        sensorI2C_mstrControl    = (uint8) mode;

        slaveAddress = (sensorI2C_GET_I2C_8BIT_ADDRESS(slaveAddress) | sensorI2C_I2C_READ_FLAG);

        sensorI2C_mstrStatus &= (uint16) ~sensorI2C_I2C_MSTAT_RD_CMPLT;
        ...
        /* Prepare reading */
        if(sensorI2C_mstrRdBufSize < sensorI2C_I2C_FIFO_SIZE) /* Reading byte-by-byte */
        {
            sensorI2C_SetRxInterruptMode(sensorI2C_INTR_RX_NOT_EMPTY);
        }
        else /* Receive RX FIFO chunks */
        {
            sensorI2C_ENABLE_MASTER_AUTO_DATA_ACK;
...
/*******************************************************************************
* Function Name: sensorI2C_I2CMasterSendStart
********************************************************************************
*
* Summary:
...
uint32 sensorI2C_I2CMasterSendStart(uint32 slaveAddress, uint32 bitRnW)
{
    ...
    if(sensorI2C_CHECK_I2C_FSM_IDLE)
    {
        ...
        if(sensorI2C_CHECK_I2C_STATUS(sensorI2C_I2C_STATUS_BUS_BUSY))
        {
            errStatus = sensorI2C_I2C_MSTR_BUS_BUSY;
        }
        else
        {
            sensorI2C_DisableInt();  /* Lock from interruption */
            ...
            else /* Read direction */
            {
                sensorI2C_state = sensorI2C_I2C_FSM_MSTR_RD_DATA;
                         slaveAddress |= sensorI2C_I2C_READ_FLAG;
            }

            /* TX and RX FIFO have to be EMPTY */

            sensorI2C_TX_FIFO_WR_REG = slaveAddress; /* Put address in TX FIFO */
            sensorI2C_ClearMasterInterruptSource(sensorI2C_INTR_MASTER_ALL);

            sensorI2C_I2C_MASTER_GENERATE_START;


            while(!sensorI2C_CHECK_INTR_MASTER(sensorI2C_INTR_MASTER_I2C_ACK      |
                                                      sensorI2C_INTR_MASTER_I2C_NACK     |
                                                      sensorI2C_INTR_MASTER_I2C_ARB_LOST |
            ...
            if(sensorI2C_CHECK_INTR_MASTER(sensorI2C_INTR_MASTER_I2C_ACK))
            {
                errStatus = sensorI2C_I2C_MSTR_NO_ERROR;
            }
            else if(sensorI2C_CHECK_INTR_MASTER(sensorI2C_INTR_MASTER_I2C_NACK))
            {
                errStatus = sensorI2C_I2C_MSTR_ERR_LB_NAK;
            }
            else if(sensorI2C_CHECK_INTR_MASTER(sensorI2C_INTR_MASTER_I2C_ARB_LOST))
            {
                sensorI2C_state = sensorI2C_I2C_FSM_IDLE;
                             errStatus = sensorI2C_I2C_MSTR_ERR_ARB_LOST;
                             resetIp   = sensorI2C_I2C_RESET_ERROR;
            }
            else /* sensorI2C_INTR_MASTER_I2C_BUS_ERROR set is else condition */
            {
                sensorI2C_state = sensorI2C_I2C_FSM_IDLE;
                             errStatus = sensorI2C_I2C_MSTR_ERR_BUS_ERR;
                             resetIp   = sensorI2C_I2C_RESET_ERROR;
            }

            sensorI2C_ClearMasterInterruptSource(sensorI2C_INTR_MASTER_I2C_ACK      |
                                                        sensorI2C_INTR_MASTER_I2C_NACK     |
                                                        sensorI2C_INTR_MASTER_I2C_ARB_LOST |
                                                        sensorI2C_INTR_MASTER_I2C_BUS_ERROR);
...
/*******************************************************************************
* Function Name: sensorI2C_I2CMasterSendRestart
********************************************************************************
*
* Summary:
...
uint32 sensorI2C_I2CMasterSendRestart(uint32 slaveAddress, uint32 bitRnW)
{
    ...
    if(sensorI2C_CHECK_I2C_MASTER_ACTIVE)
    {
        slaveAddress = sensorI2C_GET_I2C_8BIT_ADDRESS(slaveAddress);
        ...
        */
        ...
        while(sensorI2C_CHECK_I2C_MASTER_CMD(sensorI2C_I2C_MASTER_CMD_M_START))
        {
...
/*******************************************************************************
* Function Name: sensorI2C_I2CMasterSendStop
********************************************************************************
*
* Summary:
...
uint32 sensorI2C_I2CMasterSendStop(void)
{
    ...
    if(sensorI2C_CHECK_I2C_MASTER_ACTIVE)
    {
        ...
        */
        ...
        while(!sensorI2C_CHECK_INTR_MASTER(sensorI2C_INTR_MASTER_I2C_STOP     |
                                                  sensorI2C_INTR_MASTER_I2C_ARB_LOST |
                                                  sensorI2C_INTR_MASTER_I2C_BUS_ERROR))
        {
        ...
        if(sensorI2C_CHECK_INTR_MASTER(sensorI2C_INTR_MASTER_I2C_STOP))
        {
            errStatus = sensorI2C_I2C_MSTR_NO_ERROR;
        }
        else if(sensorI2C_CHECK_INTR_MASTER(sensorI2C_INTR_MASTER_I2C_ARB_LOST))
        ...
        else /* sensorI2C_INTR_MASTER_I2C_BUS_ERROR is set */
        {
            errStatus = sensorI2C_I2C_MSTR_ERR_BUS_ERR;
            resetIp   = sensorI2C_I2C_RESET_ERROR;
        }
...
/*******************************************************************************
* Function Name: sensorI2C_I2CMasterWriteByte
********************************************************************************
*
* Summary:
...
uint32 sensorI2C_I2CMasterWriteByte(uint32 theByte)
{
    ...
    if(sensorI2C_CHECK_I2C_MASTER_ACTIVE)
    {
        sensorI2C_TX_FIFO_WR_REG = theByte;
        ...
        else /* sensorI2C_INTR_MASTER_I2C_BUS_ERROR set is */
        {
            sensorI2C_state = sensorI2C_I2C_FSM_IDLE;
                         errStatus = sensorI2C_I2C_MSTR_ERR_BUS_ERR;
                         resetIp   = sensorI2C_I2C_RESET_ERROR;
...
/*******************************************************************************
* Function Name: sensorI2C_I2CMasterReadByte
********************************************************************************
*
* Summary:
...
uint32 sensorI2C_I2CMasterReadByte(uint32 ackNack)
{
    ...
    theByte = sensorI2C_I2C_INVALID_BYTE;

    /* Check FSM state before read byte */
    if(sensorI2C_CHECK_I2C_MASTER_ACTIVE)
    {
        while((!sensorI2C_CHECK_INTR_RX(sensorI2C_INTR_RX_NOT_EMPTY)) &&
              (!sensorI2C_CHECK_INTR_MASTER(sensorI2C_INTR_MASTER_I2C_ARB_LOST |
                                                  sensorI2C_INTR_MASTER_I2C_BUS_ERROR)))
        {
        ...
        if(sensorI2C_CHECK_INTR_RX(sensorI2C_INTR_RX_NOT_EMPTY))
        {
            theByte = sensorI2C_RX_FIFO_RD_REG;
...
/*******************************************************************************
* Function Name: sensorI2C_I2CMasterGetReadBufSize
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  sensorI2C_mstrRdBufIndex - used to current index within master read
*  buffer.
*
*******************************************************************************/
uint32 sensorI2C_I2CMasterGetReadBufSize(void)
{
    return(sensorI2C_mstrRdBufIndex);
}


/*******************************************************************************
* Function Name: sensorI2C_I2CMasterGetWriteBufSize
********************************************************************************
*
* Summary:
...
{
    return(sensorI2C_mstrWrBufIndex);
}


/*******************************************************************************
* Function Name: sensorI2C_I2CMasterClearReadBuf
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: sensorI2C_I2CMasterClearWriteBuf
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: sensorI2C_I2CMasterStatus
********************************************************************************
*
* Summary:
...
uint32 sensorI2C_I2CMasterStatus(void)
{
    ...
    status = (uint32) sensorI2C_mstrStatus;

    if (sensorI2C_CHECK_I2C_MASTER_ACTIVE)
    {
        /* Add status of master pending transaction: MSTAT_XFER_INP */
        status |= (uint32) sensorI2C_I2C_MSTAT_XFER_INP;
    }
...
/*******************************************************************************
* Function Name: sensorI2C_I2CMasterClearStatus
********************************************************************************
*
* Summary:
...
uint32 sensorI2C_I2CMasterClearStatus(void)
{
    ...
    status = (uint32) sensorI2C_mstrStatus;
    sensorI2C_mstrStatus = sensorI2C_I2C_MSTAT_CLEAR;
...
/*******************************************************************************
* Function Name: sensorI2C_I2CReStartGeneration
********************************************************************************
*
* Summary:
...
void sensorI2C_I2CReStartGeneration(void)
{
    ...
    uint32 status = sensorI2C_I2C_STATUS_REG;

    while(sensorI2C_WAIT_SDA_SET_HIGH)
    {
        /* Wait when slave release SDA line: SCL tHIGH is complete */
    }

    /* Prepare DR register to drive SCL line */
    sensorI2C_SET_I2C_SCL_DR(sensorI2C_I2C_SCL_LOW);

    /* Switch HSIOM to GPIO: SCL goes low */
    sensorI2C_SET_I2C_SCL_HSIOM_SEL(sensorI2C_HSIOM_GPIO_SEL);

    /* Disable SCB block */
    sensorI2C_CTRL_REG &= (uint32) ~sensorI2C_CTRL_ENABLED;

    if(0u != (status & sensorI2C_I2C_STATUS_M_READ))
    {
        /* Generate NACK use GPIO functionality */
        sensorI2C_SET_I2C_SCL_DR(sensorI2C_I2C_SCL_LOW);
        CyDelayUs(sensorI2C_I2C_TLOW_TIME); /* Count tLOW */

        sensorI2C_SET_I2C_SCL_DR(sensorI2C_I2C_SCL_HIGH);
        while(sensorI2C_WAIT_SCL_SET_HIGH)
        {
            /* Wait until slave releases SCL in case if it stretches */
        }
        CyDelayUs(sensorI2C_I2C_THIGH_TIME); /* Count tHIGH */
    }
    ...
    sensorI2C_I2C_MASTER_CMD_REG = sensorI2C_I2C_MASTER_CMD_M_START;

    /* Enable SCB block */
    sensorI2C_CTRL_REG |= (uint32) sensorI2C_CTRL_ENABLED;

    /* Switch HSIOM to I2C: */
    sensorI2C_SET_I2C_SCL_HSIOM_SEL(sensorI2C_HSIOM_I2C_SEL);
    ...
    cmd  = sensorI2C_I2C_MASTER_CMD_M_START;
    cmd |= sensorI2C_CHECK_I2C_STATUS(sensorI2C_I2C_STATUS_M_READ) ?
                (sensorI2C_I2C_MASTER_CMD_M_NACK) : (0u);

    sensorI2C_I2C_MASTER_CMD_REG = cmd;
    ...
    sensorI2C_I2C_MULTI_MASTER_SLAVE_CONST && sensorI2C_I2C_WAKE_ENABLE_CONST)
    /*******************************************************************************
    * Function Name: sensorI2C_I2CMasterDisableEcAm
    ********************************************************************************
    *
    * Summary:
    ...
    static void sensorI2C_I2CMasterDisableEcAm(void)
    {
        ...
        * This applicable only for Multi-Master-Slave with wakeup enabled. Ticket ID#192742 */
        if (0u != (sensorI2C_CTRL_REG & sensorI2C_CTRL_EC_AM_MODE))
        {
            ...
            sensorI2C_Stop();
            sensorI2C_CTRL_REG &= (uint32) ~sensorI2C_CTRL_EC_AM_MODE;
            sensorI2C_Enable();
        }
    }
#endif /* (!sensorI2C_CY_SCBIP_V0) */
