...
VAR_TYPE iChannel0LeftIn, iChannel1LeftIn;
// right input data from ad1836
VAR_TYPE iChannel0RightIn, iChannel1RightIn;
// left ouput data for ad1836
VAR_TYPE iChannel0LeftOut, iChannel1LeftOut;
// right ouput data for ad1836
VAR_TYPE iChannel0RightOut, iChannel1RightOut;
...
volatile VAR_TYPE iTxBuffer1[8];
// SPORT0 DMA receive buffer
volatile VAR_TYPE iRxBuffer1[8];

// init FlagAMode - hvad der skal ske ved pushbutton
short FlagAMode = FILTER_ACTIVE; //Start mode: PASS_THROUGH or FILTER_ACTIVE;
...
void main(void)
{
	...
	while(1)
	{
		if(rb_ready)
		{
			an_analyze(!rb_currActiveBuffer);
			rb_ready = false;
		}
	}
